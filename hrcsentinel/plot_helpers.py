...
def drawnow(draw_fig, show_once=False, confirm=False, stop_on_close=False,
            *args, **kwargs):
    """
    Gratefully stolen from: https://github.com/stsievert/python-drawnow
...
def scp_file_to_hrcmonitor(user='tremblay', server='kady.cfa.harvard.edu', file_to_scp='/Users/grant/Desktop/cap1603_plotly_voltages.html', destination='/proj/web-icxc/htdocs/hrcops/hrcmonitor/', verbose=True):
    '''
    SCP the plotly html file to the web server. Use SSH key pair.
    '''
    os.system(f'scp {file_to_scp} {user}@{server}:{destination}')
    if verbose:
        print(f"SCP'ed {file_to_scp} to {user}@{server}:{destination}")

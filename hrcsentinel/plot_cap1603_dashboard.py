...
def update_plot(telem_start, time_zero, iteration_count, old_telem=None, save_path=None, monitor=False):
    ...
    ax1.plot((telem['2P15VBVL'].times - time_zero.secs) / 3600,
             telem['2P15VBVL'].vals, lw=weight, label='+15V 2P15VBVL')
    ax1.plot((telem['2N15VBVL'].times - time_zero.secs) / 3600,
             telem['2N15VBVL'].vals, lw=weight, label='-15V 2N15VBVL')
    ax1.plot((telem['2P05VBVL'].times - time_zero.secs) / 3600,
             telem['2P05VBVL'].vals, lw=weight, label='+5V 2P05VBVL')
    # ax1.plot((telem['2P24VBVL'].times - time_zero.secs) / 3600,
    #          telem['2P24VBVL'].vals, lw=weight, label='+24V 2P24VBVL')
    ...
    if old_telem is not None:
        reftime = chandraDateTime(event_times.time_of_cap_1543).secs

        ax1.plot((old_telem['2P15VBVL'].times - reftime) / 3600, old_telem['2P15VBVL'].vals,
                 color='gray', alpha=0.5, zorder=1, label='2020 Side B Power-up')
        ax1.plot((old_telem['2N15VBVL'].times - reftime) / 3600, old_telem['2N15VBVL'].vals,
        ...
        ax2.plot((old_telem['2FHTRMZT'].times - reftime) / 3600,
                 old_telem['2FHTRMZT'].vals, color='gray', alpha=0.5, zorder=1)
    ...
    for ax in [ax1, ax2]:
        ...
        ax.text(0, ax.get_ylim()[
            1], 'Start of Comm & CAP 1603', ha='right', fontsize=8, color='slategray', zorder=3)
        nowtime = (CxoTime.now().secs - time_zero.secs)/3600
        ax.axvline(nowtime, color=comm_status_color, alpha=0.5)
        ax.text(nowtime, ax.get_ylim()[
                1], 'Now (' + comm_status_text + ')', fontsize=8, color=comm_status_color, zorder=3)

        ax.legend(prop={'size': 10}, loc=3)

        radmon_enable = CxoTime('2022:070:12:22:50.194').secs - time_zero.secs
        deadman = CxoTime('2022:070:14:12:30').secs - time_zero.secs
        comm_end = CxoTime('2022:070:14:15:00').secs - time_zero.secs
        ax.axvline(radmon_enable/3600, color=plot_stylers.green, alpha=0.5)
        ax.text(radmon_enable/3600, ax.get_ylim()[1], 'RadMon Enable',
                fontsize=8, color=plot_stylers.green, zorder=3)
        ax.axvline(deadman/3600, color=plot_stylers.red, alpha=0.5)
        ax.axvline(comm_end/3600, color=plot_stylers.purple, alpha=0.5)
        ax.text(deadman/3600, ax.get_ylim()
                [1], 'Deadman', color=plot_stylers.red, ha='right', fontsize=8)

        ax.text(comm_end/3600, ax.get_ylim()
                [1], 'EOT', color=plot_stylers.purple, ha='left', fontsize=8)
...
def main():
    ...
    old_telem = fetch.MSIDset(['2P15VBVL', '2N15VBVL', '2P05VBVL', '2P24VBVL', '2FHTRMZT', '2CHTRPZT'],
                              start=convert_to_doy(
        event_times.time_of_cap_1543),
        stop=convert_to_doy(
        event_times.time_of_cap_1543 + dt.timedelta(days=1)))

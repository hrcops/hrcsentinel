...
def update_plot(telem_start, time_zero, iteration_count, old_telem=None, save_path=None, monitor=False):
    ...
    for ax in [ax1, ax2]:
        ...
        ax.text(0, ax.get_ylim()[
            1], 'Start of CAP 1608', ha='right', fontsize=8, color='slategray', zorder=3)
        ...
        deadman = CxoTime('2022:140:00:34:45.000').secs - time_zero.secs
        comm_end = CxoTime('2022:140:00:35:00').secs - time_zero.secs
        maneuver = CxoTime('2022:140:00:24:57').secs - time_zero.secs
        ...
        ax.text(deadman/3600, ax.get_ylim()
                [1], 'Deadman', color=plot_stylers.red, ha='left', fontsize=8)

        ax.text(comm_end/3600, ax.get_ylim()
                [0], 'EOT', color=plot_stylers.purple, ha='right', va='top', fontsize=8)

        ax.text(maneuver/3600, ax.get_ylim()
                [1], 'S/C Maneuver', color=plot_stylers.red, ha='right', fontsize=8)
    ...
    if save_path is not None:
        ...
        if monitor is True:
            scp_file_to_hrcmonitor(
                file_to_scp=save_path, destination='/proj/web-icxc/htdocs/hrcops/hrcmonitor/plots/cap1608.png')
...
def main():
    ...
    telem_start = '2022:139'
    # time_zero = CxoTime.now()  # fake for testing
    time_zero = CxoTime('2022:139:21:15')  # CAP start
    iteration_count = 0
    ...
    while True:
        ...
        if args.monitor is True:
            save_path = f'/Users/grant/Desktop/cap1608_plots/{iteration_count}_dashboard.png'
